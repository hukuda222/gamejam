        ...
        if dir == 1:
            ...
            if key in self.loaded:
                if size == None:
                    ...
                    self.ctx.drawImage(self.images[key],pos[0],pos[1],size[0],size[1])
                else:
                    self.ctx.drawImage(self.images[key],spos[0],spos[1],ssize[0],ssize[1],pos[0],pos[1],size[0],size[1])
            self.ctx.restore()
        else:
            if key in self.loaded:
                if size == None:
...
class AudioManager:
    ...
    def stop(self,key):
        if key in self.loaded and self.audios[key].paused == False:
            self.audios[key].play()
    ...
    def manage(self,key,command):
        if key in self.loaded:
            if command == "play":
                self.audios[key].play()
            if command == "pause" and self.audios[key].paused == False:
                self.audios[key].pause()
            if command == "stop" and self.audios[key].paused == False:
                self.audios[key].pause()
                self.audios[key].currentTime = 0
